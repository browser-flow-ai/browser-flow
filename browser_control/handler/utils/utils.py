...
"""General utilities for web2str Python implementation."""
...
def is_string_type(field_type: Type) -> bool:
    ...
    return field_type == str or (
                hasattr(field_type, '__origin__') and field_type.__origin__ is Union and str in get_args(field_type))
...
def trim_trailing_text_node(path: Optional[str]) -> Optional[str]:
    """Remove trailing text() nodes from XPath."""
    if not path:
        return path
